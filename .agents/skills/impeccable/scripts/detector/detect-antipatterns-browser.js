...
(() => {
	...
	function parseRgb(color) {
		...
		return { a: m[4] === undefined ? 1 : +m[4], b: +m[3], g: +m[2], r: +m[1] };
	}
			...
			if (bgs) {
				...
				if (ratio < threshold) {
					...
					const isAlphaFallbackFP =
						!(DETECTOR_IS_BROWSER || effectiveBg) &&
						textColor.a != null &&
						textColor.a < 1;
	...
	function resolveBackground(el, win, customPropMap) {
		...
		while (current && current.nodeType === 1) {
			...
			if (bg && bg.a > 0.1 && (DETECTOR_IS_BROWSER || bg.a >= 0.5)) {
				return bg;
			}
	...
	function resolveFontSizePx(el, win) {
		...
		for (let i = chain.length - 1; i >= 0; i--) {
			...
			if (v.endsWith("px")) {
				...
				px = num; // unitless — already resolved
			}
		}
		return px;
	}
	...
	function textDescendantsFlushSides(el, rect) {
		const flush = { bottom: false, left: false, right: false, top: false };
		...
		for (const node of candidates) {
			if (
				!(TEXT_EDGE_TAGS.has(node.tagName) && hasMeaningfulDirectText(node))
			) {
				continue;
			}
			let nodeRect = null;
	...
	function checkQuality(opts) {
		...
		if (
			...
			if (
				!(inNavHeader || hasOwnBg || isPositioned) &&
				widthRatio > 0.5 &&
	...
	function isCreamColor(rgb) {
		...
		if (Math.min(r, g, b) < 209) {
			return false; // must be light
		}
		if (!(r >= g && g >= b)) {
			return false; // warm ordering
		}
		const warmth = r - b;
	...
	function isRenderedForBrowserRule(el) {
		for (let cur = el; cur && cur.nodeType === 1; cur = cur.parentElement) {
			...
			if ((Number.parseFloat(style.opacity) || 0) <= 0.01) {
				return false;
			}
